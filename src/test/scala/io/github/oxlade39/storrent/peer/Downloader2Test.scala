...
class Downloader2Test extends TestKit(ActorSystem("Downloader2Test"))
  ...
  "WorkFetcher" must {
    "fetch work and send to queue, ordered by rarest" in {
      ...
      val peer0LocalStatus = PeerProtocol.PeerStatus(choked = false, interested = true)
      val peer0RemoteStatus = PeerProtocol.PeerStatus()
      val peer1LocalStatus = PeerProtocol.PeerStatus(choked = false, interested = true)
      val peer1RemoteStatus = PeerProtocol.PeerStatus()
